...
contract BobVault is EIP1967Admin, Ownable, YieldConnector {
    ...
    uint64 internal constant MAX_FEE = 10 ** 16; // 1%
     ...
     * @param _token address of the collateral token.
     * @param _inFee fee for TOKEN->BOB buys (or 10 ** 18 to pause buys).
     * @param _outFee fee for BOB->TOKEN sells (or 10 ** 18 to pause sells).
     */
    function setCollateralFees(address _token, uint64 _inFee, uint64 _outFee) external onlyOwner {
        Collateral storage token = collateral[_token];
        require(token.price > 0, "BobVault: unsupported collateral");

        require(_inFee <= MAX_FEE || _inFee == 10 ** 18, "BobVault: invalid inFee");
        require(_outFee <= MAX_FEE || _outFee == 10 ** 18, "BobVault: invalid outFee");
    ...
    function getAmountOut(address _inToken, address _outToken, uint256 _inAmount) public view returns (uint256) {
        ...
        if (_outToken == address(bobToken)) {
            ...
            uint256 fee = _inAmount * uint256(token.inFee) / 10 ** 18;
            uint256 sellAmount = _inAmount - fee;
            uint256 outAmount = sellAmount * 10 ** 18 / token.price;
            ...
            uint256 outAmount = _inAmount * token.price / 10 ** 18;
            ...
            outAmount -= outAmount * uint256(token.outFee) / 10 ** 18;
            ...
            uint256 fee = _inAmount * uint256(inToken.inFee) / 10 ** 18;
            uint256 sellAmount = _inAmount - fee;
            uint256 bobAmount = sellAmount * 10 ** 18 / inToken.price;

            uint256 outAmount = bobAmount * outToken.price / 10 ** 18;
            // collected outFee should be available for withdrawal after the swap,
            // so collateral liquidity is checked before subtracting the fee
            require(outToken.balance >= outAmount, "BobVault: insufficient liquidity for collateral");
            outAmount -= outAmount * uint256(outToken.outFee) / 10 ** 18;
    ...
    function getAmountIn(address _inToken, address _outToken, uint256 _outAmount) public view returns (uint256) {
        ...
        if (_outToken == address(bobToken)) {
            ...
            uint256 sellAmount = _outAmount * token.price / 10 ** 18;
            uint256 inAmount = sellAmount * 10 ** 18 / (10 ** 18 - uint256(token.inFee));
            ...
            uint256 buyAmount = _outAmount * 10 ** 18 / (10 ** 18 - uint256(token.outFee));
            // collected outFee should be available for withdrawal after the swap,
            // so collateral liquidity is checked before subtracting the fee
            require(token.balance >= buyAmount, "BobVault: insufficient liquidity for collateral");

            uint256 inAmount = buyAmount * 10 ** 18 / token.price;
            ...
            uint256 buyAmount = _outAmount * 10 ** 18 / (10 ** 18 - uint256(outToken.outFee));
            // collected outFee should be available for withdrawal after the swap,
            // so collateral liquidity is checked before subtracting the fee
            require(outToken.balance >= buyAmount, "BobVault: insufficient liquidity for collateral");

            uint256 bobAmount = buyAmount * 10 ** 18 / outToken.price;
            uint256 sellAmount = bobAmount * inToken.price / 10 ** 18;
            uint256 inAmount = sellAmount * 10 ** 18 / (10 ** 18 - uint256(inToken.inFee));
    ...
    function buy(address _token, uint256 _amount) external returns (uint256) {
        ...
        uint256 fee = _amount * uint256(token.inFee) / 10 ** 18;
        uint256 sellAmount = _amount - fee;
        uint256 buyAmount = sellAmount * 10 ** 18 / token.price;
        unchecked {
            require(token.balance + sellAmount <= token.maxBalance, "BobVault: exceeds max balance");
    ...
    function sell(address _token, uint256 _amount) external returns (uint256) {
        ...
        uint256 buyAmount = _amount * token.price / 10 ** 18;
        // collected outFee should be available for withdrawal after the swap,
        // so collateral liquidity is checked before subtracting the fee
        require(token.balance >= buyAmount, "BobVault: insufficient liquidity for collateral");
        unchecked {
        ...
        buyAmount -= buyAmount * uint256(token.outFee) / 10 ** 18;
    ...
    function swap(address _inToken, address _outToken, uint256 _amount) external returns (uint256) {
        ...
        uint256 fee = _amount * uint256(inToken.inFee) / 10 ** 18;
        uint256 sellAmount = _amount - fee;
        unchecked {
            require(inToken.balance + sellAmount <= inToken.maxBalance, "BobVault: exceeds max balance");
        ...
        uint256 buyAmount = bobAmount * outToken.price / 10 ** 18;
        // collected outFee should be available for withdrawal after the swap,
        // so collateral liquidity is checked before subtracting the fee
        require(outToken.balance >= buyAmount, "BobVault: insufficient liquidity for collateral");
        unchecked {
        ...
        buyAmount -= buyAmount * uint256(outToken.outFee) / 10 ** 18;
